...
class QuizInterface():
    def __init__(self, quiz_brain: QuizBrain):
        ...
        self.ques_text = self.canvas.create_text(150, 125, text="Question HERE", width=250, font=("Arial", 20, "italic"))
    ...
    def get_next_ques(self):
        ...
        if self.quiz.still_has_questions():
            ...
            self.canvas.itemconfig(self.ques_text, text="You have reached end of quiz")
            self.true_button.config(state="disabled")
            self.false_button.config(state="disabled")

    def true_click(self):
        is_right = self.quiz.check_answer("True")[0]
        self.score_label.config(text=f"Score: {self.quiz.score}")
        self.give_feedback(is_right)


    def false_click(self):
        is_right = self.quiz.check_answer("False")[0]
        self.score_label.config(text=f"Score: {self.quiz.score}")
        self.give_feedback(is_right)
